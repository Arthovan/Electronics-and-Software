...
#include <algorithm>    //  algorithm relateds are defined here like sort, 
...
int main(){
    ...
    std::cout << "Hai" << '\n';         // "/n" will create a new line after the output, this is better in performance compared to endl
    ...
    std::getline(std::cin,fullname);    //  As we use '\n' at the end of the cin previously and these is affecting here in getline, to avoid this we need to use std::ws (whitespace)
...
#elif defined(SIZEOF)
int main(){
    //  Sizeof()    ==  deteremines the size in bytes of a variable, data type, class
    //                  objects, etc...
    double gpa = 2.3;
    int val = 5;
    char grade = 'S';
...
int main(){
    ...
    /*  Note:   If we have a same function name with same signature for 2 functions, only function body changes than compiler throws an error "redefenition" of fucntion
...
#elif defined(POINTER_REFERENCE)
//using std::format;
int main(){
    int x {7};  // x = 7 or x{7} both are same. Its just value assignment
    int* ip = &x;
    int& y = x; // this is a reference variable so its recommoned to make it as const so that the software cant change it at run time or during develepment
    // Reference is nothing but alias for the same variable but new name. So the address will be same

    std::cout << "The value of x is "<<x <<std::endl;
    std::cout << "The value of *ip is "<<*ip <<'\n';
...
int main(){
    ...
    int main(){
        ...
        std::ranges::sort(my_array);    //  Now this in in ranges library
        print(my_array);
        return 0;
    }
    ...
    int main(){
        ...
        print(my_var);
        my_var.reserve(10);     //  It will reserve a space of 10 before run time. comment and uncomment than check the size and capacity  
        for(int i=0;i<10;i++){
            std::cout<<"Size: "<<my_var.size()<<'\n';           //  Returns the number of elements the variable has
            std::cout<<"Capacity: "<<my_var.capacity()<<'\n';   //  Returns the number of elements that can be held in currently allocated storage 
            my_var.push_back(i);
        }
        return 0;
    }
